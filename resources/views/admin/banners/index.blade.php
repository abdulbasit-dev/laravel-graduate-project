                                            ...
                                            <tr>
                                                ...
                                                <td class="d-flex">
                                                    <button class="btn btn-info btn-sm" type="button" data-bs-toggle="modal"
                                                        data-bs-target="#editModal_{{ $college->id }}">Edit</button>



                                                    <a href="javascript:void(0)" class="btn btn-outline-danger btn-sm ms-2"
                                                        onclick="if (confirm('{{ __('Are you sure you went to delete this?') }}')){document.getElementById('delete-{{ $college->id }}').submit();} else{return false}">
                                                        Delete
                                                    </a>

                                                    <form action="{{ route('admin.colleges.destroy', $college) }}"
                                                        method="POST" id="delete-{{ $college->id }}">
                                                        @csrf
                                                        @method('DELETE')
                                                    </form>

                                                    <!-- Edit Modal -->
                                                    <div class="modal fade" id="editModal_{{ $college->id }}"
                                                        ...
                                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                                            <div class="modal-content">
                                                                ...
                                                                <div class="modal-body px-md-5">
                                                                    <h2 class="h4 text-center">Edit College
                                                                        {{ $college->name }}</h2>
                                                                    <form
                                                                        action="{{ route('admin.colleges.update', $college) }}"
                                                                        method="POST">
                                                                        @csrf
                                                                        @method("PATCH")
                                                                        <!-- Form -->
