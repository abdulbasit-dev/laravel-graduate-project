    ...
    <div class="row">
      <div class="col-12 col-xl-12">
        <div class="card card-body mb-4 border-0 shadow">
          <form action="{{ route('admin.project-announcements.update', $projectAnnouncement->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method("PUT")
              ...
              <div class="col-md-6 mb-3">
                ...
                <select class="form-select" id="college" name="college_id">
                  ...
                  @foreach (\App\Models\College::pluck('name', 'id') as $key => $value)
                    <option value="{{ $key }}" @if ($key == $projectAnnouncement->college_id) selected="selected" @endif>{{ $value }}</option>
                  @endforeach
                </select>
              </div>
            </div>
            ...
            <div class="row mt-3">
              <div class="col-md-8 mb-3">
                ...
                <textarea class="form-control tinymce @error('title') is-invalid @enderror" rows="6" id="title" name="title">{{ old('title', $projectAnnouncement->title) }}</textarea>
                @error('title')
